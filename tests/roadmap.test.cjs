...
`
    ...
    const result = runCanopyTools('roadmap get-phase 1', tmpDir);
...
`
    ...
    const result = runCanopyTools('roadmap get-phase 5', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
`
    ...
    const result = runCanopyTools('roadmap get-phase 2.1', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
describe('roadmap analyze command', () => {
  ...
  test('missing ROADMAP.md returns error', () => {
    const result = runCanopyTools('roadmap analyze', tmpDir);
...
`
    ...
    const p2 = path.join(tmpDir, '.canopy', 'phases', '02-authentication');
