...
describe('milestone complete command', () => {
  ...
  test('archives roadmap, requirements, creates MILESTONES.md', () => {
    ...
    const result = runCanopyTools('milestone complete v1.0 --name MVP Foundation', tmpDir);
    ...
    assert.ok(
      fs.existsSync(path.join(tmpDir, '.canopy', 'milestones', 'v1.0-ROADMAP.md')),
      'archived roadmap should exist'
    );
    assert.ok(
      fs.existsSync(path.join(tmpDir, '.canopy', 'milestones', 'v1.0-REQUIREMENTS.md')),
      'archived requirements should exist'
    );

    // Verify MILESTONES.md created
    assert.ok(
      fs.existsSync(path.join(tmpDir, '.canopy', 'MILESTONES.md')),
      'MILESTONES.md should be created'
    );
    const milestones = fs.readFileSync(path.join(tmpDir, '.canopy', 'MILESTONES.md'), 'utf-8');
  ...
  test('appends to existing MILESTONES.md', () => {
    fs.writeFileSync(
      path.join(tmpDir, '.canopy', 'MILESTONES.md'),
    ...
    const result = runCanopyTools('milestone complete v1.0 --name Beta', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
