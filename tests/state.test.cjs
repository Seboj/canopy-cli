...
describe('state-snapshot command', () => {
  ...
  test('missing STATE.md returns error', () => {
    const result = runCanopyTools('state-snapshot', tmpDir);
...
**Resume File:** .canopy/phases/03-api/03-02-PLAN.md
`
    );
    ...
    assert.strictEqual(output.session.resume_file, '.canopy/phases/03-api/03-02-PLAN.md', 'resume file extracted');
  });
...
`
    ...
    const outsideDir = fs.mkdtempSync(path.join(require('os').tmpdir(), 'canopy-test-outside-'));

    try {
      const result = runCanopyTools(`state-snapshot --cwd "${tmpDir}"`, outsideDir);
      assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('returns error for invalid --cwd path', () => {
    ...
    const result = runCanopyTools(`state-snapshot --cwd "${invalid}"`, tmpDir);
...
`
    ...
    const result = runCanopyTools("state add-blocker --text 'Waiting on vendor quote $1.00 before approval'", tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
`
    ...
    const result = runCanopyTools(`state add-blocker --text-file "${blockerPath}"`, tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
