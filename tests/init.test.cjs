...
describe('init commands', () => {
  ...
  test('init execute-phase returns file paths', () => {
    ...
    const result = runCanopyTools('init execute-phase 03', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);

    const output = JSON.parse(result.output);
    assert.strictEqual(output.state_path, '.canopy/STATE.md');
    assert.strictEqual(output.roadmap_path, '.canopy/ROADMAP.md');
    assert.strictEqual(output.config_path, '.canopy/config.json');
  });
  ...
  test('init plan-phase returns file paths', () => {
    ...
    const result = runCanopyTools('init plan-phase 03', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);

    const output = JSON.parse(result.output);
    assert.strictEqual(output.state_path, '.canopy/STATE.md');
    assert.strictEqual(output.roadmap_path, '.canopy/ROADMAP.md');
    assert.strictEqual(output.requirements_path, '.canopy/REQUIREMENTS.md');
    assert.strictEqual(output.context_path, '.canopy/phases/03-api/03-CONTEXT.md');
    assert.strictEqual(output.research_path, '.canopy/phases/03-api/03-RESEARCH.md');
    assert.strictEqual(output.verification_path, '.canopy/phases/03-api/03-VERIFICATION.md');
    assert.strictEqual(output.uat_path, '.canopy/phases/03-api/03-UAT.md');
  });

  test('init progress returns file paths', () => {
    const result = runCanopyTools('init progress', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);

    const output = JSON.parse(result.output);
    assert.strictEqual(output.state_path, '.canopy/STATE.md');
    assert.strictEqual(output.roadmap_path, '.canopy/ROADMAP.md');
    assert.strictEqual(output.project_path, '.canopy/PROJECT.md');
    assert.strictEqual(output.config_path, '.canopy/config.json');
  });
  ...
  test('init phase-op returns core and optional phase file paths', () => {
    ...
    const result = runCanopyTools('init phase-op 03', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('init plan-phase extracts phase_req_ids from ROADMAP', () => {
    ...
    const result = runCanopyTools('init plan-phase 3', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('init execute-phase extracts phase_req_ids from ROADMAP', () => {
    ...
    const result = runCanopyTools('init execute-phase 3', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
