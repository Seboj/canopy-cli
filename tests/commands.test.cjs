...
describe('history-digest command', () => {
  ...
  test('empty phases directory returns valid schema', () => {
    const result = runCanopyTools('history-digest', tmpDir);
  ...
  test('nested frontmatter fields extracted correctly', () => {
    ...
    const phaseDir = path.join(tmpDir, '.canopy', 'phases', '01-foundation');
...
`;
  ...
  test('multiple phases merged into single digest', () => {
    // Create phase 01
    const phase01Dir = path.join(tmpDir, '.canopy', 'phases', '01-foundation');
...
`
    ...
    // Create phase 02
    const phase02Dir = path.join(tmpDir, '.canopy', 'phases', '02-api');
...
describe('summary-extract command', () => {
  ...
  test('missing file returns error', () => {
    const result = runCanopyTools('summary-extract .canopy/phases/01-test/01-01-SUMMARY.md', tmpDir);
...
`
    ...
    const result = runCanopyTools('summary-extract .canopy/phases/01-foundation/01-01-SUMMARY.md', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);

    const output = JSON.parse(result.output);
    assert.strictEqual(output.path, '.canopy/phases/01-foundation/01-01-SUMMARY.md', 'path correct');
...
`
    ...
    const result = runCanopyTools('summary-extract .canopy/phases/01-foundation/01-01-SUMMARY.md --fields one_liner,key_files,requirements_completed', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
describe('progress command', () => {
  ...
  test('renders JSON progress', () => {
    ...
    const result = runCanopyTools('progress json', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('renders bar format', () => {
    ...
    const result = runCanopyTools('progress bar --raw', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('renders table format', () => {
    ...
    const result = runCanopyTools('progress table --raw', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('does not crash when summaries exceed plans (orphaned SUMMARY.md)', () => {
    ...
    const barResult = runCanopyTools('progress bar --raw', tmpDir);
    ...
    const tableResult = runCanopyTools('progress table --raw', tmpDir);
    assert.ok(tableResult.success, `Table format crashed: ${tableResult.error}`);

    // json format - percent should be clamped
    const jsonResult = runCanopyTools('progress json', tmpDir);
...
describe('todo complete command', () => {
  ...
  test('moves todo from pending to completed', () => {
    const pendingDir = path.join(tmpDir, '.canopy', 'todos', 'pending');
    ...
    const result = runCanopyTools('todo complete add-dark-mode.md', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
    ...
    // Verify moved
    assert.ok(
      !fs.existsSync(path.join(tmpDir, '.canopy', 'todos', 'pending', 'add-dark-mode.md')),
      'should be removed from pending'
    );
    assert.ok(
      fs.existsSync(path.join(tmpDir, '.canopy', 'todos', 'completed', 'add-dark-mode.md')),
      'should be in completed'
    );

    // Verify completion timestamp added
    const content = fs.readFileSync(
      path.join(tmpDir, '.canopy', 'todos', 'completed', 'add-dark-mode.md'),
      'utf-8'
    );
  ...
  test('fails for nonexistent todo', () => {
    const result = runCanopyTools('todo complete nonexistent.md', tmpDir);
...
describe('scaffold command', () => {
  ...
  test('scaffolds context file', () => {
    ...
    const result = runCanopyTools('scaffold context --phase 3', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
    ...
    // Verify file content
    const content = fs.readFileSync(
      path.join(tmpDir, '.canopy', 'phases', '03-api', '03-CONTEXT.md'),
      'utf-8'
    );
  ...
  test('scaffolds UAT file', () => {
    ...
    const result = runCanopyTools('scaffold uat --phase 3', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
    ...
    const content = fs.readFileSync(
      path.join(tmpDir, '.canopy', 'phases', '03-api', '03-UAT.md'),
      'utf-8'
    );
  ...
  test('scaffolds verification file', () => {
    ...
    const result = runCanopyTools('scaffold verification --phase 3', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
    ...
    const content = fs.readFileSync(
      path.join(tmpDir, '.canopy', 'phases', '03-api', '03-VERIFICATION.md'),
      'utf-8'
    );
  ...
  test('scaffolds phase directory', () => {
    const result = runCanopyTools('scaffold phase-dir --phase 5 --name User Dashboard', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);

    const output = JSON.parse(result.output);
    assert.strictEqual(output.created, true);
    assert.ok(
      fs.existsSync(path.join(tmpDir, '.canopy', 'phases', '05-user-dashboard')),
