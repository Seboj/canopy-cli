...
describe('validate consistency command', () => {
  ...
  test('passes for consistent project', () => {
    ...
    fs.mkdirSync(path.join(tmpDir, '.canopy', 'phases', '03-c'), { recursive: true });

    const result = runCanopyTools('validate consistency', tmpDir);
  ...
  test('warns about phase on disk but not in roadmap', () => {
    ...
    fs.mkdirSync(path.join(tmpDir, '.canopy', 'phases', '01-a'), { recursive: true });
    fs.mkdirSync(path.join(tmpDir, '.canopy', 'phases', '02-orphan'), { recursive: true });
