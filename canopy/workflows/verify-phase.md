...
<required_reading>
@~/.claude/canopy/references/verification-patterns.md
@~/.claude/canopy/templates/verification-report.md
</required_reading>
...
grep -E "^| ${phase_number}" .canopy/REQUIREMENTS.md 2>/dev/null
...
Use canopy-tools to extract must_haves from each PLAN:

```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  MUST_HAVES=$(node ~/.claude/canopy/bin/canopy-tools.cjs frontmatter get "$plan" --field must_haves)
...
```bash
PHASE_DATA=$(node ~/.claude/canopy/bin/canopy-tools.cjs roadmap get-phase "${phase_number}" --raw)
```
...
Use canopy-tools for artifact verification against must_haves in each PLAN:

```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  ARTIFACT_RESULT=$(node ~/.claude/canopy/bin/canopy-tools.cjs verify artifacts "$plan")
...
Use canopy-tools for key link verification against must_haves in each PLAN:

```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  LINKS_RESULT=$(node ~/.claude/canopy/bin/canopy-tools.cjs verify key-links "$plan")
...
See ~/.claude/canopy/templates/verification-report.md for complete template.
</step>
