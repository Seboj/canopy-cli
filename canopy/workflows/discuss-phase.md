...
1. **canopy-phase-researcher** — Reads CONTEXT.md to know WHAT to research
   - "User wants card-based layout" → researcher investigates card component patterns
   - "Infinite scroll decided" → researcher looks into virtualization libraries

2. **canopy-planner** — Reads CONTEXT.md to know WHAT decisions are locked
...
**Express path available:** If you already have a PRD or acceptance criteria document, use `/canopy:plan-phase {phase} --prd path/to/prd.md` to skip this discussion and go straight to planning.
...
Use /canopy:progress to see available phases.
```
Exit workflow.
...
- options:
  - "Continue and replan after" — Capture context, then run /canopy:plan-phase {X} to replan
...
```
Created: .canopy/phases/${PADDED_PHASE}-${SLUG}/${PADDED_PHASE}-CONTEXT.md
...
**Also available:**
- `/canopy:plan-phase ${PHASE} --skip-research` — plan without research
...
```bash
node ~/.claude/canopy/bin/canopy-tools.cjs commit "docs(${padded_phase}): capture phase context" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
```bash
node ~/.claude/canopy/bin/canopy-tools.cjs commit "docs(state): record phase ${PHASE} context session" --files .canopy/STATE.md
```
</step>
...
Task(
  prompt="
    ...
    <execution_context>
    @~/.claude/canopy/workflows/plan-phase.md
    ...
    3. When spawning agents (canopy-phase-researcher, canopy-planner, canopy-plan-checker), use Task with specified subagent_type and model
    ...
    5. Do NOT use the Skill tool or /canopy: commands. Read workflow .md files directly.
...
- **PLANNING COMPLETE** → Planning done, execution didn't complete:
  ...
  Continue: /canopy:execute-phase ${PHASE}
  ```
- **PLANNING INCONCLUSIVE / CHECKPOINT** → Stop chain:
  ```
  Auto-advance stopped: Planning needs input.
  Continue: /canopy:plan-phase ${PHASE}
  ```
- **GAPS FOUND** → Stop chain:
  ```
  Auto-advance stopped: Gaps found during execution.
  Continue: /canopy:plan-phase ${PHASE} --gaps
  ```
