<purpose>
Execute small, ad-hoc tasks with Canopy guarantees (atomic commits, STATE.md tracking). Quick mode spawns canopy-planner (quick mode) + canopy-executor(s), tracks tasks in `.canopy/quick/`, and updates STATE.md's "Quick Tasks Completed" table.
...
```bash
INIT=$(node ~/.claude/canopy/bin/canopy-tools.cjs init quick "$DESCRIPTION")
```
...
**If `roadmap_exists` is false:** Error — Quick mode requires an active project with ROADMAP.md. Run `/canopy:new-project` first.
...
```bash
QUICK_DIR=".canopy/quick/${next_num}-${slug}"
mkdir -p "$QUICK_DIR"
```
...
Spawn canopy-executor with plan reference:
...
```bash
node ~/.claude/canopy/bin/canopy-tools.cjs commit "docs(quick-${next_num}): ${DESCRIPTION}" --files ${file_list}
```
...
CANOPY > QUICK TASK COMPLETE (FULL MODE)
...
Ready for next task: /canopy:quick
```
...
- [ ] Directory created at `.canopy/quick/NNN-slug/`
