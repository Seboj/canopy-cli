...
```bash
INIT=$(node ~/.claude/canopy/bin/canopy-tools.cjs init milestone-op)
```
...
```bash
CHECKER_MODEL=$(node ~/.claude/canopy/bin/canopy-tools.cjs resolve-model canopy-integration-checker --raw)
```
...
node ~/.claude/canopy/bin/canopy-tools.cjs phases list
```
...
PHASE_INFO=$(node ~/.claude/canopy/bin/canopy-tools.cjs find-phase 01 --raw)
...
Verify cross-phase wiring and E2E user flows.",
  subagent_type="canopy-integration-checker",
...
for summary in .canopy/phases/*-*/*-SUMMARY.md; do
  node ~/.claude/canopy/bin/canopy-tools.cjs summary-extract "$summary" --fields requirements_completed | jq -r '.requirements_completed'
done
```
...
Create `.canopy/v{version}-v{version}-MILESTONE-AUDIT.md` with:
...
**Report:** .canopy/v{version}-MILESTONE-AUDIT.md
...
**Also available:**
- cat .canopy/v{version}-MILESTONE-AUDIT.md — see full report
- /canopy:complete-milestone {version} — proceed anyway (accept tech debt)
