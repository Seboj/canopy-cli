...
cat .canopy/PROJECT.md 2>/dev/null
```
...
```bash
ls .canopy/phases/XX-current/*-PLAN.md 2>/dev/null | sort
ls .canopy/phases/XX-current/*-SUMMARY.md 2>/dev/null | sort
```
...
```bash
ls .canopy/phases/XX-current/.continue-here*.md 2>/dev/null
```
...
**Delegate ROADMAP.md and STATE.md updates to canopy-tools:**

```bash
TRANSITION=$(node ~/.claude/canopy/bin/canopy-tools.cjs phase complete "${current_phase}")
```
...
```bash
cat .canopy/phases/XX-current/*-SUMMARY.md
```
...
**Note:** Basic position updates (Current Phase, Status, Current Plan, Last Activity) were already handled by `canopy-tools phase complete` in the update_roadmap_and_state step.

Verify the updates are correct by reading STATE.md. If the progress bar needs updating, use:

```bash
PROGRESS=$(node ~/.claude/canopy/bin/canopy-tools.cjs progress bar --raw)
```
...
**Use the transition result from `canopy-tools phase complete`:**
...
```bash
ls .canopy/phases/*[X+1]*/*-CONTEXT.md 2>/dev/null
```
...
Exit skill and invoke SlashCommand("/canopy:plan-phase [X+1] --auto")
...
Exit skill and invoke SlashCommand("/canopy:discuss-phase [X+1] --auto")
...
`/canopy:discuss-phase [X+1]` — gather context and clarify approach
...
**Also available:**
- `/canopy:plan-phase [X+1]` — skip discussion, plan directly
- `/canopy:research-phase [X+1]` — investigate unknowns
...
**Also available:**
- `/canopy:discuss-phase [X+1]` — revisit context
- `/canopy:research-phase [X+1]` — investigate unknowns
...
```bash
node ~/.claude/canopy/bin/canopy-tools.cjs config-set workflow.auto_advance false
```
...
Exit skill and invoke SlashCommand("/canopy:complete-milestone {version}")
