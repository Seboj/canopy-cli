...
1. Extract full milestone details to `.canopy/milestones/v[X.Y]-ROADMAP.md`
2. Archive requirements to `.canopy/milestones/v[X.Y]-REQUIREMENTS.md`
...
```bash
ROADMAP=$(node ~/.claude/canopy/bin/canopy-tools.cjs roadmap analyze)
```
...
2. **Run audit first** — `/canopy:audit-milestone` to assess gap severity
...
```bash
cat .canopy/config.json 2>/dev/null
```
...
for summary in .canopy/phases/*-*/*-SUMMARY.md; do
  node ~/.claude/canopy/bin/canopy-tools.cjs summary-extract "$summary" --fields one_liner | jq -r '.one_liner'
done
```
...
**Note:** MILESTONES.md entry is now created automatically by `canopy-tools milestone complete` in the archive_milestone step. The entry includes version, date, phase/plan/task counts, and accomplishments extracted from SUMMARY.md files.
...
Update `.canopy/ROADMAP.md` — group completed milestone phases:
...
**Delegate archival to canopy-tools:**

```bash
ARCHIVE=$(node ~/.claude/canopy/bin/canopy-tools.cjs milestone complete "v[X.Y]" --name "[Milestone Name]")
```

The CLI handles:
- Creating `.canopy/milestones/` directory
...
Verify: `✅ Milestone archived to .canopy/milestones/`
...
```bash
mkdir -p .canopy/milestones/v[X.Y]-phases
# For each phase directory in .canopy/phases/:
mv .canopy/phases/{phase-dir} .canopy/milestones/v[X.Y]-phases/
```
Verify: `✅ Phase directories archived to .canopy/milestones/v[X.Y]-phases/`

If "Skip": Phase directories remain in `.canopy/phases/` as raw execution history. Use `/canopy:cleanup` later to archive retroactively.
...
```bash
rm .canopy/ROADMAP.md
rm .canopy/REQUIREMENTS.md
```
...
```bash
ls .canopy/RETROSPECTIVE.md 2>/dev/null
```

**If exists:** Read the file, append new milestone section before the "## Cross-Milestone Trends" section.

**If doesn't exist:** Create from template at `~/.claude/canopy/templates/retrospective.md`.
...
**Commit:**
```bash
node ~/.claude/canopy/bin/canopy-tools.cjs commit "docs: update retrospective for v${VERSION}" --files .canopy/RETROSPECTIVE.md
```
...
See: .canopy/PROJECT.md (updated [today])
...
```bash
INIT=$(node ~/.claude/canopy/bin/canopy-tools.cjs init execute-phase "1")
```
...
if [ "$BRANCHING_STRATEGY" = "phase" ]; then
  for branch in $PHASE_BRANCHES; do
    ...
    # Strip .canopy/ from staging if commit_docs is false
    if [ "$COMMIT_DOCS" = "false" ]; then
      git reset HEAD .canopy/ 2>/dev/null || true
    fi
...
See .canopy/MILESTONES.md for full details."
```
...
```bash
node ~/.claude/canopy/bin/canopy-tools.cjs commit "chore: complete v[X.Y] milestone" --files .canopy/milestones/v[X.Y]-ROADMAP.md .canopy/milestones/v[X.Y]-REQUIREMENTS.md .canopy/milestones/v[X.Y]-MILESTONE-AUDIT.md .canopy/MILESTONES.md .canopy/PROJECT.md .canopy/STATE.md
```
```
...
- [ ] User knows next step (/canopy:new-milestone)
