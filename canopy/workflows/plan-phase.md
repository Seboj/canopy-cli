<purpose>
Create executable phase prompts (PLAN.md files) for a roadmap phase with integrated research and verification. Default flow: Research (if needed) -> Plan -> Verify -> Done. Orchestrates canopy-phase-researcher, canopy-planner, and canopy-plan-checker agents with a revision loop (max 3 iterations).
</purpose>
...
```bash
INIT=$(node ~/.claude/canopy/bin/canopy-tools.cjs init plan-phase "$PHASE")
```
...
**If `planning_exists` is false:** Error — run `/canopy:new-project` first.
...
```bash
mkdir -p ".canopy/phases/${padded_phase}-${phase_slug}"
```
...
5. Commit:
```bash
node ~/.claude/canopy/bin/canopy-tools.cjs commit "docs(${padded_phase}): generate context from PRD" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
If "Run discuss-phase first": Display `/canopy:discuss-phase {X}` and exit workflow.
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 CANOPY ► RESEARCHING PHASE {X}
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

◆ Spawning researcher...
```

### Spawn canopy-phase-researcher

```bash
PHASE_DESC=$(node ~/.claude/canopy/bin/canopy-tools.cjs roadmap get-phase "${PHASE}" | jq -r '.section')
```
...
```
Task(
  prompt="First, read ~/.claude/agents/canopy-phase-researcher.md for your role and instructions.\n\n" + research_prompt,
...
**If found:**
1. Read validation template from `~/.claude/canopy/templates/VALIDATION.md`
...
```bash
node ~/.claude/canopy/bin/canopy-tools.cjs commit-docs "docs(phase-${PHASE}): add validation strategy"
```
...
## 8. Spawn canopy-planner Agent

Display banner:
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 CANOPY ► PLANNING PHASE {X}
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
<downstream_consumer>
Output consumed by /canopy:execute-phase. Plans need:
...
```
Task(
  prompt="First, read ~/.claude/agents/canopy-planner.md for your role and instructions.\n\n" + filled_prompt,
  subagent_type="general-purpose",
  model="{planner_model}",
...
## 10. Spawn canopy-plan-checker Agent

Display banner:
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 CANOPY ► VERIFYING PLANS
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
Task(
  ...
  subagent_type="canopy-plan-checker",
  model="{checker_model}",
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 CANOPY ► AUTO-ADVANCING TO EXECUTE
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
Task(
  prompt="
    ...
    <execution_context>
    @~/.claude/canopy/workflows/execute-phase.md
    ...
    4. When spawning executor agents, use subagent_type='canopy-executor' with the existing @file pattern from the workflow
    5. When spawning verifier agents, use subagent_type='canopy-verifier'
    6. Preserve the classifyHandoffIfNeeded workaround (spot-check on that specific error)
    7. Do NOT use the Skill tool or /canopy: commands
    </instructions>
  ",
  subagent_type="general-purpose",
...
- **PHASE COMPLETE** → Display final summary:
  ```
  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
   CANOPY ► PHASE ${PHASE} COMPLETE ✓
  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

  Auto-advance pipeline finished.

  Next: /canopy:discuss-phase ${NEXT_PHASE} --auto
  ```
...
**Also available:**
- cat .canopy/phases/{phase-dir}/*-PLAN.md — review plans
- /canopy:plan-phase {X} --research — re-research first

───────────────────────────────────────────────────────────────
</offer_next>

<success_criteria>
- [ ] .canopy/ directory validated
- [ ] Phase validated against roadmap
- [ ] Phase directory created if needed
- [ ] CONTEXT.md loaded early (step 4) and passed to ALL agents
- [ ] Research completed (unless --skip-research or --gaps or exists)
- [ ] canopy-phase-researcher spawned with CONTEXT.md
- [ ] Existing plans checked
- [ ] canopy-planner spawned with CONTEXT.md + RESEARCH.md
- [ ] Plans created (PLANNING COMPLETE or CHECKPOINT handled)
- [ ] canopy-plan-checker spawned with CONTEXT.md
