...
@~/.claude/canopy/references/git-integration.md
</required_reading>
...
```bash
INIT=$(node ~/.claude/canopy/bin/canopy-tools.cjs init execute-phase "${PHASE}")
```
...
If `.canopy/` missing: error.
</step>

<step name="identify_plan">
```bash
# Use plans/summaries from INIT JSON, or list files
ls .canopy/phases/XX-name/*-PLAN.md 2>/dev/null | sort
ls .canopy/phases/XX-name/*-SUMMARY.md 2>/dev/null | sort
```
...
# config settings can be fetched via canopy-tools config-get if needed
```
...
```bash
grep -n "type=\"checkpoint" .canopy/phases/XX-name/{phase}-{plan}-PLAN.md
```
...
**Pattern A:** init_agent_tracking → spawn Task(subagent_type="canopy-executor", model=executor_model) with prompt: execute plan at [path], autonomous, all tasks + SUMMARY + commit, follow deviation/auth rules, report: plan name, tasks, SUMMARY path, commit hash → track agent_id → wait → update tracking → report.
...
```bash
if [ ! -f .canopy/agent-history.json ]; then
  echo '{"version":"1.0","max_entries":50,"entries":[]}' > .canopy/agent-history.json
fi
rm -f .canopy/current-agent-id.txt
if [ -f .canopy/current-agent-id.txt ]; then
  INTERRUPTED_ID=$(cat .canopy/current-agent-id.txt)
...
2. Per segment:
   - Subagent route: spawn canopy-executor for assigned tasks only. Prompt: task range, plan path, read full plan for context, execute assigned tasks, track deviations, NO SUMMARY/commit. Track via agent protocol.
...
```bash
cat .canopy/phases/XX-name/{phase}-{plan}-PLAN.md
```
...
```bash
node ~/.claude/canopy/bin/canopy-tools.cjs phases list --type summaries --raw
...
See `~/.claude/canopy/references/tdd.md` for structure.
</tdd_plan_execution>
...
See ~/.claude/canopy/references/checkpoints.md for details.
</step>
...
```bash
grep -A 50 "^user_setup:" .canopy/phases/XX-name/{phase}-{plan}-PLAN.md | head -50
```

If user_setup exists: create `{phase}-USER-SETUP.md` using template `~/.claude/canopy/templates/user-setup.md`. Per service: env vars table, account setup checklist, dashboard config, local dev notes, verification commands. Status "Incomplete". Set `USER_SETUP_CREATED=true`. If empty/missing: skip.
</step>

<step name="create_summary">
Create `{phase}-{plan}-SUMMARY.md` at `.canopy/phases/XX-name/`. Use `~/.claude/canopy/templates/summary.md`.
...
Update STATE.md using canopy-tools:

```bash
# Advance plan counter (handles last-plan edge case)
node ~/.claude/canopy/bin/canopy-tools.cjs state advance-plan

# Recalculate progress bar from disk state
node ~/.claude/canopy/bin/canopy-tools.cjs state update-progress

# Record execution metrics
node ~/.claude/canopy/bin/canopy-tools.cjs state record-metric \
...
node ~/.claude/canopy/bin/canopy-tools.cjs state add-decision \
  --phase "${PHASE}" --summary-file "${DECISION_TEXT_FILE}" --rationale-file "${RATIONALE_FILE}"

# Add blockers if any found
node ~/.claude/canopy/bin/canopy-tools.cjs state add-blocker --text-file "${BLOCKER_TEXT_FILE}"
```
</step>

<step name="update_session_continuity">
Update session info using canopy-tools:

```bash
node ~/.claude/canopy/bin/canopy-tools.cjs state record-session \
...
```bash
node ~/.claude/canopy/bin/canopy-tools.cjs roadmap update-plan-progress "${PHASE}"
```
...
```bash
node ~/.claude/canopy/bin/canopy-tools.cjs requirements mark-complete ${REQ_IDS}
```
...
```bash
node ~/.claude/canopy/bin/canopy-tools.cjs commit "docs({phase}-{plan}): complete [plan-name] plan" --files .canopy/phases/XX-name/{phase}-{plan}-SUMMARY.md .canopy/STATE.md .canopy/ROADMAP.md .canopy/REQUIREMENTS.md
```
</step>

<step name="update_codebase_map">
If .canopy/codebase/ doesn't exist: skip.
...
```bash
ls -1 .canopy/phases/[current-phase-dir]/*-PLAN.md 2>/dev/null | wc -l
ls -1 .canopy/phases/[current-phase-dir]/*-SUMMARY.md 2>/dev/null | wc -l
```

| Condition | Route | Action |
|-----------|-------|--------|
| summaries < plans | **A: More plans** | Find next PLAN without SUMMARY. Yolo: auto-continue. Interactive: show next plan, suggest `/canopy:execute-phase {phase}` + `/canopy:verify-work`. STOP here. |
| summaries = plans, current < highest phase | **B: Phase done** | Show completion, suggest `/canopy:plan-phase {Z+1}` + `/canopy:verify-work {Z}` + `/canopy:discuss-phase {Z+1}` |
| summaries = plans, current = highest phase | **C: Milestone done** | Show banner, suggest `/canopy:complete-milestone` + `/canopy:verify-work` + `/canopy:add-phase` |
