...
Debug files use the `.canopy/debug/` path (hidden directory with leading dot).
</paths>
...
- truth: "Comment appears immediately after submission"
  ...
  debug_session: .canopy/debug/comment-not-refreshing.md
```
...
```bash
node ~/.claude/canopy/bin/canopy-tools.cjs commit "docs({phase_num}): add root causes from diagnosis" --files ".canopy/phases/XX-name/{phase_num}-UAT.md"
```
</step>
