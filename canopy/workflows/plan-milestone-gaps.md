<purpose>
Create all phases necessary to close gaps identified by `/canopy:audit-milestone`. Reads MILESTONE-AUDIT.md, groups gaps into logical phases, creates phase entries in ROADMAP.md, and offers to plan each phase. One command creates all fix phases — no manual `/canopy:add-phase` per gap.
</purpose>
...
ls -t .canopy/v*-MILESTONE-AUDIT.md 2>/dev/null | head -1
```
...
```
No audit gaps found. Run `/canopy:audit-milestone` first.
```
...
PHASES=$(node ~/.claude/canopy/bin/canopy-tools.cjs phases list)
...
grep -c "Pending" .canopy/REQUIREMENTS.md
```

## 8. Create Phase Directories

```bash
mkdir -p ".canopy/phases/{NN}-{name}"
```

## 9. Commit Roadmap and Requirements Update

```bash
node ~/.claude/canopy/bin/canopy-tools.cjs commit "docs(roadmap): add gap closure phases {N}-{M}" --files .canopy/ROADMAP.md .canopy/REQUIREMENTS.md
```
...
**Also available:**
- `/canopy:execute-phase {N}` — if plans already exist
- `cat .canopy/ROADMAP.md` — see updated roadmap

---

**After all gap phases complete:**

`/canopy:audit-milestone` — re-audit to verify gaps closed
`/canopy:complete-milestone {version}` — archive when audit passes
```
...
- [ ] User knows to run `/canopy:plan-phase` next
</success_criteria>
