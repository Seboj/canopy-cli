...
```bash
PHASE_INFO=$(node ~/.claude/canopy/bin/canopy-tools.cjs find-phase "${PHASE}")
```
...
```bash
PHASE_CHECK=$(node ~/.claude/canopy/bin/canopy-tools.cjs roadmap get-phase "${PHASE}")
...
```bash
PHASE_DIR=$(node ~/.claude/canopy/bin/canopy-tools.cjs find-phase "${PHASE}" --raw)
```
