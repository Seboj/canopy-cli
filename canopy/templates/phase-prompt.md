...
> **Note:** Planning methodology is in `agents/canopy-planner.md`.
> This template defines the PLAN.md output format that the agent produces.

Template for `.canopy/phases/XX-name/{phase}-{plan}-PLAN.md` - executable phase plans optimized for parallel execution.
...
<!-- For checkpoint task examples and patterns, see @~/.claude/canopy/references/checkpoints.md -->
...
**Wave is pre-computed:** Wave numbers are assigned during `/canopy:plan-phase`. Execute-phase reads `wave` directly from frontmatter and groups plans by wave number. No runtime dependency analysis needed.
...
```markdown
<context>
@.canopy/phases/03-features/03-01-SUMMARY.md  # Just because it's earlier
@.canopy/phases/03-features/03-02-SUMMARY.md  # Reflexive chaining
</context>
```
...
See `~/.claude/canopy/references/tdd.md` for TDD plan structure.
...
<output>
After completion, create `.canopy/phases/03-features/03-01-SUMMARY.md`
</output>
```
...
@.canopy/PROJECT.md
@.canopy/ROADMAP.md
@.canopy/phases/03-features/03-01-SUMMARY.md
@.canopy/phases/03-features/03-02-SUMMARY.md
</context>
...
<output>
After completion, create `.canopy/phases/03-features/03-03-SUMMARY.md`
</output>
```
...
See `~/.claude/canopy/templates/user-setup.md` for full schema and examples
...
See `~/.claude/canopy/workflows/verify-phase.md` for verification logic.
