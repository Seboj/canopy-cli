...
| CRITICAL | <= 25% | Stop immediately, save state (`/canopy:pause-work`) |
...
```
Statusline Hook (canopy-statusline.js)
    | writes
    v
/tmp/claude-ctx-{session_id}.json
    ^ reads
    |
Context Monitor (canopy-context-monitor.js, PostToolUse)
    | injects
    v
...
Canopy's `/canopy:pause-work` command saves execution state. The WARNING message suggests using it. The CRITICAL message instructs immediate state save.

## Setup

Both hooks are automatically registered during `npx canopy-cc` installation:
...
```json
{
  "statusLine": {
    "type": "command",
    "command": "node ~/.claude/hooks/canopy-statusline.js"
  },
  "hooks": {
    "PostToolUse": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "node ~/.claude/hooks/canopy-context-monitor.js"
          }
        ]
      }
    ]
